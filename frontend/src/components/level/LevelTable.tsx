...
export interface LevelTableProps {
  levels: any[];
  onEdit: (lvl: any) => void;
  onDelete: (lvl: any) => void;
  loading: boolean;
}

export default function LevelTable({ levels, onEdit, onDelete, loading }: LevelTableProps) {
  if (loading) {
    return (
      <div className="text-center py-12 border border-dashed border-slate-200 rounded-xl bg-white text-slate-500 font-medium">
        Loading employee levels...
      </div>
    );
  }
      ...
      <table className="w-full text-left text-sm text-slate-600 border-collapse">
        <thead className="bg-slate-50 border-b border-slate-200">
          <tr>
            ...
            <th className="px-6 py-4 font-semibold text-slate-800">Level Name</th>
        ...
        <tbody className="divide-y divide-slate-100">
          {levels.map((lvl) => (
            <tr key={lvl.level_id} className="hover:bg-slate-50/50 transition-colors">
              ...
              <td className="px-6 py-4 font-semibold text-slate-900">{lvl.level_name}</td>
              <td className="px-6 py-4 text-slate-600">{lvl.level_description || '—'}</td>
              <td className="px-6 py-4 text-slate-500">
                {lvl.created_at ? new Date(lvl.created_at).toLocaleDateString('vi-VN') : '—'}
              </td>
