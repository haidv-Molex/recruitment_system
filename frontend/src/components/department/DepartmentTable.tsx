...
export default function DepartmentTable({ departments, onEdit, onDelete, loading }: DepartmentTableProps) {
  if (loading) {
    return (
      ...
      <table className="w-full text-left text-sm text-slate-600 border-collapse">
        <thead className="bg-slate-50 border-b border-slate-200">
          <tr>
            ...
            <th className="px-6 py-4 font-semibold text-slate-800">Department Name</th>
        ...
        <tbody className="divide-y divide-slate-100">
          {departments.map((dept) => (
            <tr key={dept.department_id} className="hover:bg-slate-50/50 transition-colors">
              ...
              <td className="px-6 py-4 font-semibold text-slate-900">{dept.department_name}</td>
              <td className="px-6 py-4 text-slate-600">{dept.department_description || '—'}</td>
              <td className="px-6 py-4 text-slate-500">
                {dept.created_at ? new Date(dept.created_at).toLocaleDateString('vi-VN') : '—'}
              </td>
