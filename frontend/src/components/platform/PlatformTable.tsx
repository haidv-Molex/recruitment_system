...
export default function PlatformTable({ platforms, onEdit, onDelete, loading }: PlatformTableProps) {
  if (loading) {
    return (
      <div className="text-center py-12 border border-dashed border-slate-200 rounded-xl bg-white text-slate-500 font-medium">
        Loading recruitment platforms...
      </div>
    );
  }
  ...
  if (platforms.length === 0) {
    return (
      <div className="text-center py-12 border border-dashed border-slate-200 rounded-xl bg-white">
        <p className="text-slate-400 text-sm">No recruitment platforms found</p>
      </div>
    );
  }
      ...
      <table className="w-full text-left text-sm text-slate-600 border-collapse">
        <thead className="bg-slate-50 border-b border-slate-200">
          <tr>
            ...
            <th className="px-6 py-4 font-semibold text-slate-800">Platform Name</th>
        ...
        <tbody className="divide-y divide-slate-100">
          {platforms.map((platform) => (
            <tr key={platform.platform_id} className="hover:bg-slate-50/50 transition-colors">
              ...
              <td className="px-6 py-4 font-semibold text-slate-900">{platform.platform_name}</td>
              <td className="px-6 py-4 text-slate-600">{platform.platform_description || '—'}</td>
              <td className="px-6 py-4 text-slate-500">
                {platform.created_at ? new Date(platform.created_at).toLocaleDateString('vi-VN') : '—'}
              </td>
