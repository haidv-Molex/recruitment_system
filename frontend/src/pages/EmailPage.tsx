...
import { Code2, Eye, Mail, Send, LogOut, FileText } from 'lucide-react';
...
const renderOfferTemplate = (value: string, fields: OfferLetterFields) =>
  value.replace(/{{\s*(candidate_name|position|start_date)\s*}}/g, (_, key: keyof OfferLetterFields) => fields[key] || '');
...
export function EmailPage() {
  ...
  const handleSend = async () => {
    ...
    setSending(true);
    try {
      await sendEmailApi({ to, subject: renderedSubject, html: renderedHtml });
          ...
          <div className="mt-4 space-y-2">
            {templates.map((template) => (
              ...
              >
                ...
                <div className="mt-1 text-xs text-slate-500">{template.subject}</div>
              </button>
            ))}
          </div>
        </div>
        ...
        <div className="rounded-xl border border-slate-100 bg-white p-5 shadow-sm">
          ...
          <div className="mt-4">
            ...
            {showHtmlSource ? (
              ...
              <div className="min-h-[360px] rounded-lg border border-slate-200 bg-slate-50 p-4">
                {html.trim() ? (
                  <div
                    className="mx-auto min-h-[300px] max-w-3xl rounded-lg bg-white p-8 text-sm text-slate-700 shadow-sm"
          ...
          <div className="mt-4 flex justify-end">
            <Button
              onClick={handleSend}
              isLoading={sending}
              disabled={!to.trim() || !renderedSubject.trim() || !renderedHtml.trim()}
              icon={<Send size={16} />}
            >
              Send Email
            </Button>
