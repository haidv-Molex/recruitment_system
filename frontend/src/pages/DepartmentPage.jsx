...
import { Plus, Edit2, Trash2, Search, X, Layers, ChevronLeft, ChevronRight } from 'lucide-react';
...
export const DepartmentPage = () => {
  ...
  const s = {
    ...
    codeBadge: { display: 'inline-block', padding: '3px 12px', borderRadius: '6px', fontSize: '13px', fontWeight: 700, background: '#eff6ff', color: '#2563eb', border: '1px solid #bfdbfe', letterSpacing: '0.5px' },
  ...
  return (
    <div style={s.page}>
      ...
      <div style={s.headerRow}>
        <div>
          <h1 style={s.title}><Layers size={28} /> Department Management</h1>
          <p style={s.subtitle}>Manage departments in the recruitment system.</p>
        </div>
      ...
      <div style={s.filterBar}>
        ...
        {searchQuery && (
          <button type="button" style={s.clearBtn} onClick={handleClearSearch}>Clear</button>
        )}
      </div>
      ...
      <table style={s.table}>
        ...
        <tbody>
          {loading ? (
            <tr><td colSpan={5} style={s.loadingRow}>Loading departments...</td></tr>
          ) : departments.length === 0 ? (
            <tr>
              <td colSpan={5} style={s.emptyRow}>
                {searchQuery ? `No departments match "${searchQuery}".` : 'No departments yet. Click "Add Department" to create one.'}
              </td>
            </tr>
          ) : (
              ...
              <tr key={dept.id}>
                ...
                <td style={{ ...s.td, textAlign: 'center' }}>
                  <div style={{ display: 'flex', gap: '6px', justifyContent: 'center' }}>
                    ...
                    <button type="button" style={{ ...s.actionBtn, color: '#ef4444' }} onClick={() => handleDelete(dept)} title="Delete"><Trash2 size={16} /></button>
                  </div>
                </td>
              </tr>
            ))
        ...
        <div style={s.backdrop}>
          <div style={s.modal}>
            <div style={s.modalHeader}>
              <h2 style={s.modalTitle}>{editingDept ? 'Edit Department' : 'Add New Department'}</h2>
            ...
            <form onSubmit={handleSubmit}>
              <div style={s.modalBody}>
                ...
                <div style={s.row}>
                  <div style={{ ...s.fieldGroup, ...s.half }}>
                    ...
                    <input style={s.input} name="code" value={formData.code} onChange={handleFormChange} placeholder="e.g. HR" disabled={saving} autoFocus />
                    <p style={s.hint}>Short code, e.g. HR, QC, AS, LOG</p>
                  </div>
                  <div style={{ ...s.fieldGroup, ...s.half }}>
                    <label style={s.label}>Department Name *</label>
                    <input style={s.input} name="name" value={formData.name} onChange={handleFormChange} placeholder="e.g. Human Resource" disabled={saving} />
                  </div>
                </div>
                <div style={s.fieldGroup}>
                  <label style={s.label}>Description</label>
                  <textarea style={s.textarea} name="description" value={formData.description} onChange={handleFormChange} placeholder="e.g. Phòng ban dành cho HR" rows={3} disabled={saving} />
                </div>
              </div>
              <div style={s.modalFooter}>
                ...
                <button type="submit" style={s.saveBtn(saving)} disabled={saving}>
                  {saving ? (editingDept ? 'Saving...' : 'Creating...') : (editingDept ? 'Save Changes' : 'Create Department')}
                </button>
              </div>
            </form>
          </div>
