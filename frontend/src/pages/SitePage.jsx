...
import { Plus, Edit2, Trash2, Search, X, MapPin, ChevronLeft, ChevronRight } from 'lucide-react';
...
export const SitePage = () => {
  ...
  const s = {
    ...
    codeBadge: { display: 'inline-block', padding: '3px 12px', borderRadius: '6px', fontSize: '13px', fontWeight: 700, background: '#f0fdf4', color: '#16a34a', border: '1px solid #bbf7d0', letterSpacing: '0.5px' },
  ...
  return (
    <div style={s.page}>
      ...
      <div style={s.headerRow}>
        <div>
          <h1 style={s.title}><MapPin size={28} /> Site Management</h1>
          <p style={s.subtitle}>Manage factory sites / locations (MXV, D, S, SK, etc.)</p>
        </div>
      ...
      <div style={s.filterBar}>
        <div style={s.searchWrap}>
          ...
          <input style={s.searchInput} type="text" placeholder="Search by code or site name..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} onKeyDown={handleSearchKeyDown} />
        </div>
      ...
      <table style={s.table}>
        ...
        <tbody>
          {loading ? (
            <tr><td colSpan={5} style={s.loadingRow}>Loading sites...</td></tr>
          ) : sites.length === 0 ? (
            <tr>
              <td colSpan={5} style={s.emptyRow}>
                {searchQuery ? `No sites match "${searchQuery}".` : 'No sites yet. Click "Add Site" to create one.'}
              </td>
            </tr>
          ) : (
            sites.map((site) => (
              <tr key={site.site_id}>
                ...
                <td style={{ ...s.td, textAlign: 'center' }}>
                  <div style={{ display: 'flex', gap: '6px', justifyContent: 'center' }}>
                    ...
                    <button type="button" style={{ ...s.actionBtn, color: '#ef4444' }} onClick={() => handleDelete(site)} title="Delete"><Trash2 size={16} /></button>
                  </div>
                </td>
              </tr>
            ))
        ...
        <div style={s.backdrop}>
          <div style={s.modal}>
            <div style={s.modalHeader}>
              <h2 style={s.modalTitle}>{editingSite ? 'Edit Site' : 'Add New Site'}</h2>
            ...
            <form onSubmit={handleSubmit}>
              <div style={s.modalBody}>
                ...
                <div style={s.row}>
                  <div style={{ ...s.fieldGroup, ...s.half }}>
                    ...
                    <input style={s.input} name="code" value={formData.code} onChange={handleFormChange} placeholder="e.g. MXV" disabled={saving} autoFocus />
                    <p style={s.hint}>Short code, e.g. MXV, D, S, SK</p>
                  </div>
                  <div style={{ ...s.fieldGroup, ...s.half }}>
                    <label style={s.label}>Site Name *</label>
                    <input style={s.input} name="name" value={formData.name} onChange={handleFormChange} placeholder="e.g. Molex Vietnam" disabled={saving} />
                  </div>
                </div>
                <div style={s.fieldGroup}>
                  <label style={s.label}>Description</label>
                  <textarea style={s.textarea} name="description" value={formData.description} onChange={handleFormChange} placeholder="e.g. Nhà máy sản xuất tại Hà Nội" rows={3} disabled={saving} />
                </div>
              </div>
              <div style={s.modalFooter}>
                ...
                <button type="submit" style={s.saveBtn(saving)} disabled={saving}>
                  {saving ? (editingSite ? 'Saving...' : 'Creating...') : (editingSite ? 'Save Changes' : 'Create Site')}
                </button>
              </div>
            </form>
          </div>
