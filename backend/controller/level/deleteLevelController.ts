...
const paramsSchema = Joi.object({
  ...
  ids: Joi.alternatives().try(
    ...
    "any.only": "Danh sách mã cấp bậc không hợp lệ"
  })
}).or('id', 'ids');
  ...
  async (req, res) => {
    ...
    if (ids.length === 0) {
      return res.status(400).json({
        result: false,
        message: "Không có mã cấp bậc nào được cung cấp để xóa"
      });
    }

    await withTransaction(async (pool) => {
      await Level.delete(ids, pool);
    });

    res.status(200).json({
      result: true,
      message: ids.length === 1 ? "Xóa cấp bậc thành công" : "Xóa các cấp bậc thành công"
    });
  }
);
