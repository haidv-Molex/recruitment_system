...
const deleteNoteController = express.Router({ mergeParams: true });
  ...
  async (req, res) => {
    ...
    if (ids.length === 0) {
      return res.status(400).json({
        result: false,
        message: "Không có mã note nào được cung cấp để xóa"
      });
    }

    await withTransaction(async (pool) => {
      await Note.delete(ids, req.user!.user_id, pool);
    });

    res.status(200).json({
      result: true,
      message: ids.length === 1 ? "Xóa note thành công" : "Xóa các note thành công"
    });
  }
);
